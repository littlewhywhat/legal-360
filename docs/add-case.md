...
Engine owns skins (`email` / `slack` / `docs` / `system` / `aurora`) and `DemoPlayer`. Phone chrome has a ‹ back control (scene stack) on every case. Case owns only script + copy. New UI skin → extend `apps/web/components/demo/`, not a new app.
