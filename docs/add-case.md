...
Engine owns skins (`email` / `slack` / `docs` / `system` / `devices` / `devices-play`) and `DemoPlayer`. Case owns only script + copy. New UI skin → extend `apps/web/components/demo/`, not a new app.
